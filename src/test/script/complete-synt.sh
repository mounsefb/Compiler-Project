...
#Les Tests Valides
echo
printf  "\e[1;33m--------------- VALIDES SYNTAX TESTS ---------------\e[0m"
echo
echo
...
for i in ./src/test/deca/syntax/valid/*/*/*.deca 
    ...
    output_path="$OBTAINED_RES_PATH/$(basename "${i%.deca}synt.res")"
    test_synt "$i" > "$output_path"
    
    
    if test_synt "$i" 2>&1 | \
    (grep -q -e "$(basename $i):[1-9][0-9]*:[0-9][0-9]*:" || grep -q -e 'Erreur' || grep -q -e 'Exception'  )
    then
...
echo
printf  "\e[1;33m--------------- RESULTS : VALIDE SYNTAX TESTS ---------------\e[0m"
echo
echo
...
#Les Tests InValides
echo
printf  "\e[1;33m--------------- INVALIDES SYNTAX TESTS ---------------\e[0m"
echo
echo
...
echo
printf  "\e[1;33m--------------- RESULTS : INVALIDE SYNTAX TESTS ---------------\e[0m"
echo
echo
echo "PASSED TESTS : $compteurTestCorrect"
echo "FAILED TESTS : $compteurTestIncorrect"
